...
export const VideoCall = () => {
  ...
  const handleStartCall = async (data) => {
    ...
    try {
      ...
      // No usar JWT con meet.jit.si (servidor público)
      // Solo pasar el nombre de la sala y el nombre del usuario
      const roomName = encodeURIComponent(response.room_name);
      const userName = encodeURIComponent(data.user_name);
      const jitsiUrl = `https://meet.jit.si/${roomName}?userInfo.displayName=${userName}`;
